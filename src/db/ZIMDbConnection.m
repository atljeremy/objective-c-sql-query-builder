	...
	@synchronized(self) {
		if (!_isConnected) {
			if (sqlite3_open([_dataSource UTF8String], &_database) != SQLITE_OK) {
				...
				@throw [NSException exceptionWithName: @"ZIMDbException" reason: [NSString stringWithFormat: @"Failed to open database connection. '%@'", sqlite3_errmsg16(_database)] userInfo: nil];
			}
			_isConnected = YES;
		}
	}
...
- (NSNumber *) execute: (NSString *)sql {
	...
	if (sqlite3_exec(_database, [sql UTF8String], NULL, nil, nil) != SQLITE_OK) {
		...
		@throw [NSException exceptionWithName: @"ZIMDbException" reason: [NSString stringWithFormat: @"Failed to execute SQL statement. '%s'", sqlite3_errmsg(_database)] userInfo: nil];
	}
...
- (NSArray *) query: (NSString *)sql asObject: (Class)model {
	...
	if (sqlite3_prepare_v2(_database, [sql UTF8String], -1, &statement, NULL) != SQLITE_OK) {
		...
		@throw [NSException exceptionWithName: @"ZIMDbException" reason: [NSString stringWithFormat: @"Failed to perform query with SQL statement. '%@'", sqlite3_errmsg16(_database)] userInfo: nil];
	}
	...
	@synchronized(self) {
		if (_isConnected) {
			if (sqlite3_close(_database) != SQLITE_OK) {
				@throw [NSException exceptionWithName: @"ZIMDbException" reason: [NSString stringWithFormat: @"Failed to close database connection. '%@'", sqlite3_errmsg16(_database)] userInfo: nil];
			}
			_isConnected = NO;
		}
	}
